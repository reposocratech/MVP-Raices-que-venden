...
class AdminDal {
  ...
  getTextsFromUser = async (user_id) => {
    try {
      let sql =
        'SELECT text.*, user.* FROM text LEFT JOIN user ON text.user_id = user.user_id WHERE user.user_id=?';
  ...
  saveText = async ({ text_id, text_title, text_body, last_modified }) => {
    try {
      let sql =
        'UPDATE text SET text_title=?, text_body=?, last_modified=? WHERE text_id=?';
  ...
  createService = async (data) => {
    ...
    try {
      ...
      let sql2 =
        'INSERT INTO service(service_id, service_name, service_description, service_price, service_image) VALUES(?,?,?,?,?)';
  ...
  modifyService = async (data) => {
    ...
    try {
      ...
      let sql1 =
        'UPDATE service SET service_name=?, service_description=?, service_price=? WHERE service_id=?';
      if (data.service_image) {
        ...
        sql1 =
          'UPDATE service SET service_name=?, service_description=?, service_price=?, service_image=? WHERE service_id=?';
      }
  ...
  getMessage = async (user_id_client, user_id_admin) => {
    try {
